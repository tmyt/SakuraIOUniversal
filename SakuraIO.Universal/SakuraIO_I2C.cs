...
namespace SakuraIO
{
    public class SakuraIO_I2C : SakuraIO
    {
        ...
        public async Task OpenAsync(string busName)
        {
            var aqs = I2cDevice.GetDeviceSelector(busName);
            var dis = await DeviceInformation.FindAllAsync(aqs);
            var settings = new I2cConnectionSettings(SAKURAIO_SLAVE_ADDR) {BusSpeed = I2cBusSpeed.FastMode};
            _i2c = await I2cDevice.FromIdAsync(dis[0].Id, settings);
        }
    }
}
